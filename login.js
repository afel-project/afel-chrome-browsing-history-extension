...
function getDatasetInfo(username, password){
    ...
    postJsonResource(config.catalogue_base_url+"newuserdataset", "username="+username+"&password="+password+"&type=AFEL Chrome Browsing Extension&description=Collects data about browsing activities from the chrome web browser.&ecapiconf="+encodeURIComponent(ecapiconfig), function (text){
	var res=JSON.parse(text);	
	if (res.error) {displayError(res.error);}
	else if (res.key && res.dataset){
	    chrome.storage.sync.set({"acbh_dataset_id": res.dataset, "acbh_user_key": res.key, "acbh_ecapi": res.ecapi}, 
				    function (){
